...
import {View, TouchableOpacity, Text, StyleSheet, FlatList, TextInput, Button, Keyboard} from "react-native";
...
export default function SearchScreen({navigation}) {
    ...
    const searchFilterFunction = (text) => {
        ...
        if (text === '') {
            ...
            setBooks(books.filter(b => b.title.toLowerCase().includes(text.toLowerCase())))
        }
    }

    const renderCategory = ({ item }) => (
        <TouchableOpacity 
            style={[styles.category, {backgroundColor: selectedCategory === item.name ? '#FFFAF3' : '#F56C26'}]}
            onPress={() => changeCategory(item.name)}
            >
            <Text style={[styles.categoryName, {color: selectedCategory === item.name ? '#F56C26' : 'white'}]}>{item.name}</Text>
        </TouchableOpacity>
    );
